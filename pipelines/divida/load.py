...
def clear_s3_prefix(s3_client, prefix):
    print(f"-> Executando Fresh Start em: s3://{BUCKET_NAME}/{prefix}")
...
def upload_layer(layer_name):
    layer_path = os.path.join(DATA_DIR, layer_name, "divida")
    s3_prefix = f"{layer_name}/divida"
...
def run():
    ...
    print(f"-> Upload Dívida concluído. {total} arquivos enviados.")
